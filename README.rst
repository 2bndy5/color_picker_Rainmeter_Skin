.. image:: https://github.com/2bndy5/color_picker_Rainmeter_Skin/workflows/Anti-virus%20Check/badge.svg
    :target: https://github.com/2bndy5/color_picker_Rainmeter_Skin/actions?query=workflow%3A%22Anti-virus+Check%22

.. image:: https://github.com/2bndy5/color_picker_Rainmeter_Skin/workflows/RMSKIN%20Packager/badge.svg
    :target: https://github.com/2bndy5/color_picker_Rainmeter_Skin/actions?query=workflow%3A%22RMSKIN+Packager%22

.. image:: https://img.shields.io/github/v/release/rainmeter/rainmeter?label=Rainmeter&style=plastic&logo=data:image/svg+xml;base64,
    :alt: Rainmeter Required (latest stable)
    :target: https://github.com/rainmeter/rainmeter/releases/latest

.. image:: https://img.shields.io/github/license/2bndy5/color_picker_Rainmeter_Skin?color=blue&style=plastic&logo=data:image/svg%2bxml;base64,
    :alt: license
    :target: https://github.com/2bndy5/color_picker_Rainmeter_Skin/blob/master/LICENSE

color picker
############

This is the home for developing color picker. A proof-of-concept color picker that I use for all my other Rainmeter skins' settings.

.. image:: https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/i/9d354760-79b7-44eb-9a4c-5fc831468200/d5zb596-f33290e9-b027-4560-a32c-b6e22558fa27.png
    :target: https://www.deviantart.com/2bndy5/art/RGB-and-Hex-Color-Code-Picker-Version-3-361637466

Rainmeter required
##################

The minimum required version of Rainmeter is 4.1, but the `latest stable release <https://www.rainmeter.net>`_ is recommended.

Package Installer
#################

`This repository's releases <https://github.com/
2bndy5/color_picker_Rainmeter_Skin/
releases>`_ include a Rainmeter package file called
``color picker_<version>.rmskin``. Open this file
with Rainmeter, follow the prompts, and the packaged files will be
copied/installed appropriately to your Rainmeter skins folder.


Manual Install
##############

Download this repository's zip file and extract the folder
``color picker`` (located in ``Skins``
folder) into the Rainmeter skins folder (defualts to
``C:\Users\%USERNAME%\Documents\Rainmeter\skins`` -
unless onedrive manages your user documents folder). If
installing a Rainmeter layout, the aforementioned
`Package Installer`_ is preferred and highly recommended.